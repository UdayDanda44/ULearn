...
const Mainpage = () => {
  ...
  const getnofusers = async()=>{
      const {data} = await axios.get('http://minip-seven.vercel.app/api/auth/getallusers');
      setdetails(data)
  }
  const getVideos = async()=>{
    const {data} = await axios.get('http://minip-seven.vercel.app/api/auth/getallvideos')
    setvideos(data)
  }
  useEffect(() => {
    getnofusers()
